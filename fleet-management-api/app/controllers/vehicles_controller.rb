    ...
    if vehicle.nil?
      ...
      vehicle.transaction do
        shipments_info[:route].each do |r|
          @delivery_point = DeliveryPoint.find_by_id(r[:delivery_point])
          if delivery_point.nil?
            return render json: { error: 'Delivery Point not found' }, status: :not_found
          else
            r[:deliveries].each do |delivery|
              ...
              next unless package_shipment.present?
